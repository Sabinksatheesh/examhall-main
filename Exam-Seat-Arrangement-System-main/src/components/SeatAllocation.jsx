...
export default function SeatAllocation() {
  ...
  const handleExcels = async () => {
    ...
    if (!selectedDate || !selectedTime) {
      alert("Please select both date and time before sending emails.");
      setLoading(false);
      return;
    }
  
    const fileName = `${selectedDate}_${selectedTime}.xlsx`;
  
    try {
      await axiosPrivate.get('/seat-allocation/send-excels', {
        params: { files: [fileName] },
        withCredentials: true
      });
  
      alert("Seat arrangement sent successfully!");
    } catch (error) {
